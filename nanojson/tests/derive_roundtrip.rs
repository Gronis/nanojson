...
where
    T: Serialize + for<'s, 'b> Deserialize<'s> + core::fmt::Debug + PartialEq,
{
...
fn sum_leaf_integers(json: &mut Parser) -> i64 {
...
fn test_parse_as_closure() {
    let src = br#"{"x":3,"y":4}"#;
    let p = nanojson::parse_as::<Point>(src, |parser| Point::deserialize(parser)).unwrap();
...
fn test_parse_sized_as_closure() {
    let src = br#"{"x":3,"y":4}"#;
    let p = nanojson::parse_sized_as::<Point>(&mut [0; 256], src, |parser| Point::deserialize(parser)).unwrap();
    assert_eq!(p, Point { x: 3, y: 4 });
}
